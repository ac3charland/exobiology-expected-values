...
const processedSpecies = Object.keys(speciesDetailsData).map((species) => {
  ...
  variants.forEach((variant) => {
    ...
    if (isInOrionSpur) {
      ...
      if (variant.histograms.body_types) {
        Object.entries(variant.histograms.body_types).forEach(
          ([type, count]) => {
      ...
      if (variant.histograms.atmos_types) {
        Object.entries(variant.histograms.atmos_types).forEach(
          ([type, count]) => {
  ...
  const totalBodyTypeCounts = Object.values(bodyTypeCounts).reduce(
    (a, b) => a + b,
    0
  );
  const totalAtmosphereCounts = Object.values(atmosphereCounts).reduce(
    (a, b) => a + b,
    0
  );
...
species.forEach(
  ({ error, primaryStars, reward, species, bodyTypes, atmospheres }) => {
    if (!error) {
...
const processedData = Object.entries(starTypeData)
  .map(([name, data]) => ({
    ...
    speciesContributions: data.speciesContributions.sort(
...
processedData.forEach(
  ({ name, expectedValue, totalOccurrences, speciesContributions }) => {
      ...
      speciesContributions.forEach(
        ({
          ...
          bodyTypes.forEach(({ name, count, percentage }) => {
            console.log(
              `      - ${name}: ${count} (${(percentage * 100).toFixed(1)}%)`
            );
          });
